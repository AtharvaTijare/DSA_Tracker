...
export default function App() {
  ...
  const weeklyPct       = Math.min(Math.round(solvedThisWeek / 5 * 100), 100);
  ...
  const getQuality      = (t) => { const q = revQuality[t]; if (!q) return null; const tot = q.easy + q.hard; if (!tot) return null; return Math.round(q.easy / tot * 100); };
  ...
  const checkMilestone = (nc, shownList) => { const m = MILESTONES.find((ms) => ms.count === nc && !shownList.includes(ms.count)); if (m) { setTimeout(() => { setMilestoneModal(m); setMilestonesShown((p) => [...p, m.count]); boom(); }, 900); } };
  ...
  const delP        = (id) => setProblems((prev) => prev.filter((p) => p.id !== id));
  ...
  const TrackerTab = () => (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
        <Card bg={timerRunning ? dark ? "#1a0a00" : "#fff7ed" : dark ? T.cardBg : "white"} border={timerRunning ? "#f97316" : T.cardBorder}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4 }}>Solve Timer <span style={{ fontSize: 11, fontWeight: 400, color: T.subtext }}>— aim 15–20 mins</span></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <button onClick={resetTimer} style={{ background: T.mutedBg, color: T.subtext, border: "1px solid " + T.cardBorder, borderRadius: 9, padding: "10px 18px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Save & Reset</button>
            </div>
          </div>
        </Card>
      ...
      {trackerView === "grouped" && filtered.length > 0 && (() => {
        ...
        return Object.entries(grouped).map(([sec, subSecs]) => {
          ...
          return (
            <div key={sec} style={{ marginBottom: 14, borderRadius: 12, overflow: "hidden", border: "1.5px solid " + (dark ? "#2d2d4e" : "#e0e7ff") }}>
              {/* Section header */}
              <div onClick={() => toggleSection(sec)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", background: dark ? "#1e1b4b" : "linear-gradient(135deg,#4f46e5,#7c3aed)", color: "white", cursor: "pointer", userSelect: "none" }}>
                <span style={{ fontSize: 14, transition: "transform 0.2s", display: "inline-block", transform: secOpen ? "rotate(90deg)" : "rotate(0deg)" }}>▶</span>
                <span style={{ fontSize: 15 }}>📂</span>
                <span style={{ fontWeight: 700, fontSize: 14, flex: 1 }}>{sec}</span>
                <span style={{ background: "rgba(255,255,255,0.2)", borderRadius: 99, padding: "2px 10px", fontSize: 11, fontWeight: 700 }}>{solvedInSec}/{totalInSec}</span>
              </div>
              {secOpen && Object.entries(subSecs).map(([sub, probs]) => {
                ...
                return (
                  <div key={sub}>
                    {/* Sub-section header */}
                    <div onClick={() => toggleSubSec(subKey)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: dark ? "#1a1a2e" : "#eef2ff", borderTop: "1px solid " + (dark ? "#2d2d4e" : "#c7d2fe"), cursor: "pointer", userSelect: "none" }}>
                      <span style={{ fontSize: 11, transition: "transform 0.2s", display: "inline-block", transform: subOpen ? "rotate(90deg)" : "rotate(0deg)", color: dark ? "#a78bfa" : "#4f46e5" }}>▶</span>
                      <span style={{ fontSize: 13 }}>📄</span>
                      <span style={{ fontWeight: 700, fontSize: 12, color: dark ? "#a78bfa" : "#4f46e5", flex: 1 }}>{sub}</span>
                      <span style={{ fontSize: 11, color: dark ? "#a78bfa" : "#6366f1", fontWeight: 600 }}>{solvedInSub}/{probs.length} solved</span>
                      {masteredInSub > 0 && <span style={{ fontSize: 10, background: dark ? "#14281f" : "#d1fae5", color: "#059669", borderRadius: 5, padding: "1px 7px", fontWeight: 700 }}>👑 {masteredInSub}</span>}
                    </div>
                    {subOpen && (
                      <div style={{ padding: "8px 12px", background: dark ? T.cardBg : "white" }}>
                        {probs.map((p) => {
                          const rm = REV_META[p.revStage] || REV_META[0], ct = TCd(p.topic);
                          return (
  ...
  const StatsTab = () => (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        ...
        <div style={{ gridColumn: "1 / -1", background: "#161b22", borderRadius: 16, padding: "20px 24px", marginBottom: 14 }}>
          {/* Header stats */}
          <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
            <div style={{ color: "white", fontSize: 15, fontWeight: 700 }}>
              <span style={{ fontSize: 20, fontWeight: 800 }}>{yearHeatTotalActions}</span>
              <span style={{ color: "#8b949e", fontWeight: 400, marginLeft: 6 }}>actions in {heatYear}</span>
            </div>
            <div style={{ marginLeft: "auto", display: "flex", gap: 18, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ color: "#8b949e", fontSize: 12 }}>Active days: <span style={{ color: "#39d353", fontWeight: 700 }}>{activeDaysCount}</span></span>
              <span style={{ color: "#8b949e", fontSize: 12 }}>Max streak: <span style={{ color: "#39d353", fontWeight: 700 }}>{maxStreakCount}</span></span>
              <span style={{ color: "#8b949e", fontSize: 12 }}>Current: <span style={{ color: "#39d353", fontWeight: 700 }}>{streak.count}</span></span>
              <select value={heatYear} onChange={(e) => setHeatYear(Number(e.target.value))} style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid #30363d", fontSize: 12, background: "#21262d", color: "#c9d1d9", fontWeight: 700, cursor: "pointer" }}>
                {availableYears.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
          </div>

          {/* Month-separated grid */}
          {(() => {
            // Group weeks by month
            const MONTH_NAMES = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
            const monthGroups = [];
            let curMonth = -1, curGroup = null;
            yearHeatWeeks.forEach((week) => {
              // Find dominant month of this week (the month of the first in-year day)
              const inYearDays = week.filter(d => d.startsWith(String(heatYear)));
              if (inYearDays.length === 0) return;
              const mth = new Date(inYearDays[0]).getMonth();
              if (mth !== curMonth) {
                curGroup = { month: mth, weeks: [] };
              ...
              <div style={{ overflowX: "auto" }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 6, minWidth: "max-content" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 3, paddingTop: 20, marginRight: 4 }}>
                    {["", "Mon", "", "Wed", "", "Fri", ""].map((d, i) => (
                      <div key={i} style={{ fontSize: 9, color: "#8b949e", height: 13, display: "flex", alignItems: "center" }}>{d}</div>
                    ))}
                  </div>
                  {/* Month groups */}
                  {monthGroups.map((grp, gi) => (
                    <div key={gi} style={{ display: "flex", flexDirection: "column" }}>
                      {/* Month label */}
                      <div style={{ fontSize: 11, color: "#8b949e", fontWeight: 600, marginBottom: 4, height: 16 }}>
                        {MONTH_NAMES[grp.month]}
                      </div>
                      {/* Weeks in this month */}
                      <div style={{ display: "flex", gap: 3 }}>
                        {grp.weeks.map((week, wi) => (
                          <div key={wi} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                            {week.map((date) => {
                              const inYear = date.startsWith(String(heatYear));
                              const v = activityLog[date] || 0;
                              const bg = !inYear ? "transparent"
                                : v === 0 ? "#21262d"
                                : v / maxAct < 0.25 ? "#0e4429"
                                : v / maxAct < 0.5  ? "#006d32"
                                : v / maxAct < 0.75 ? "#26a641"
                                : "#39d353";
                              return (
                                <div key={date}
                                  title={inYear ? date + ": " + v + " action" + (v !== 1 ? "s" : "") : ""}
                                  style={{ width: 13, height: 13, borderRadius: 3, background: bg, border: inYear ? "1px solid rgba(255,255,255,0.06)" : "none", transition: "background 0.15s", cursor: inYear && v ? "pointer" : "default" }}
                                />
                              );
                            })}
                          </div>
          ...
          {/* Legend */}
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#8b949e", marginTop: 12 }}>
            <span>Less</span>
            {["#21262d", "#0e4429", "#006d32", "#26a641", "#39d353"].map((bg, i) => (
              <div key={i} style={{ width: 12, height: 12, borderRadius: 3, background: bg, border: "1px solid rgba(255,255,255,0.06)" }} />
            ))}
            <span>More</span>
          </div>
        </div>
  ...
  return (
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: T.pageBg, color: T.text }}>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", top: 20, right: 20, background: toast.bg, border: "2px solid " + toast.border, borderRadius: 12, padding: "12px 20px", fontWeight: 700, fontSize: 14, zIndex: 9999, animation: "slR 0.3s ease", color: "#1a1a1a", boxShadow: "0 6px 24px rgba(0,0,0,0.15)" }}>
          {toast.msg}
        </div>
      )}
      ...
      {/* MAIN */}
      <div style={{ marginLeft: 240, flex: 1, padding: "28px 36px", maxWidth: "calc(100vw - 240px)", minHeight: "100vh" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: T.subtext, marginTop: 3 }}>
            ...
            {tab === "today"    && " · " + solvedThisWeek + "/5 problems this week"}
